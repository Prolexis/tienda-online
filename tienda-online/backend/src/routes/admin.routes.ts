...
adminRoutes.use(autenticar, requireRole('ADMIN', 'GERENTE_VENTAS', 'GERENTE_INVENTARIO', 'VENDEDOR'));
    ...
    try {
      ...
      const [
        ...
        prisma.orden.aggregate({
          _sum: { total: true },
          where: { estado: 'PAGADA', createdAt: { gte: inicioMesActual } }
        }),
        prisma.orden.aggregate({
          _sum: { total: true },
          where: { estado: 'PAGADA', createdAt: { gte: inicioMesAnterior, lte: finMesAnterior } }
        }),
        prisma.orden.count({
          where: { createdAt: { gte: inicioMesActual } }
        }),
        prisma.orden.count({
      ...
      const variacion = totalMesAnterior === 0 ? 100 : ((totalMesActual - totalMesAnterior) / totalMesAnterior) * 100;
      ...
      const ventasPorCategoria = ventasPorCategoriaRaw.map(c => ({
        nombre: c.nombre,
        total: c.productos.reduce((acc, p) => acc + p.itemsOrden.reduce((sum, item) => sum + Number(item.subtotal), 0), 0)
      })).filter(c => c.total > 0);
      ...
      res.json({
        ...
        data: {
          kpis: {
            ...
            ticketPromedio: (ordenesMesActual === 0 ? 0 : totalMesActual / ordenesMesActual).toFixed(2),
...
// GET — Listar todas las categorías (con conteo de productos)
adminRoutes.get('/categories',
...
adminRoutes.post('/categories',
  ...
  async (req, res, next) => {
    try {
      const { nombre, descripcion } = z.object({
...
adminRoutes.put('/categories/:id',
  requireRole('ADMIN', 'GERENTE_INVENTARIO'),
  async (req, res, next) => {
    try {
      const id = parseInt(req.params.id);
      const { nombre, descripcion, activo } = z.object({
...
adminRoutes.delete('/categories/:id',
  requireRole('ADMIN'),
  async (req, res, next) => {
    try {
      const id = parseInt(req.params.id);
...
// GET — Listar todas las marcas (con categorías y conteo)
adminRoutes.get('/brands',
  requireRole('ADMIN', 'GERENTE_INVENTARIO'),
  async (_req, res, next) => {
    try {
...
// POST — Crear marca
adminRoutes.post('/brands',
  requireRole('ADMIN', 'GERENTE_INVENTARIO'),
  async (req, res, next) => {
    try {
      const { nombre, descripcion, imagen, categoriaIds } = z.object({
        nombre: z.string().min(1),
        descripcion: z.string().optional(),
...
adminRoutes.put('/brands/:id',
  requireRole('ADMIN', 'GERENTE_INVENTARIO'),
  async (req, res, next) => {
    try {
      const id = parseInt(req.params.id);
      const { nombre, descripcion, imagen, activo, categoriaIds } = z.object({
        nombre: z.string().optional(),
        descripcion: z.string().optional(),
        imagen: z.string().optional(),
        activo: z.boolean().optional(),
...
// PUT — Actualizar roles de un usuario
adminRoutes.put('/users/:id/roles',
  requireRole('ADMIN'),
  async (req, res, next) => {
    try {
      const userId = parseInt(req.params.id);
      const { rolId } = z.object({
        rolId: z.number().int()
      }).parse(req.body);

      // En este sistema, un usuario puede tener múltiples roles, 
      // pero el frontend parece estar simplificando a uno por ahora.
      // Vamos a resetear y poner el nuevo.
      ...
      res.json({ success: true, message: 'Roles actualizados' });
    } catch (err) { next(err); }
  }
);

// PUT — Activar/Desactivar usuario
adminRoutes.put('/users/:id/toggle',
  requireRole('ADMIN'),
  async (req, res, next) => {
    try {
      const id = parseInt(req.params.id);
...
adminRoutes.post('/products/upload',
  requireRole('ADMIN', 'GERENTE_INVENTARIO'),
...
adminRoutes.post('/brands/upload',
  requireRole('ADMIN', 'GERENTE_INVENTARIO'),
  upload.single('logo'),
  processImage,
  async (req, res) => {
...
adminRoutes.post('/inventory/restock',
  requireRole('ADMIN', 'GERENTE_INVENTARIO'),
  async (req: AuthRequest, res, next) => {
    try {
      const { productoId, cantidad, motivo } = z.object({
      ...
      if (!producto) { res.status(404).json({ success: false, message: 'Producto no encontrado' }); return; }
      ...
      res.json({ success: true, data: resultado, message: 'Stock reabastecido' });
    } catch (err) { next(err); }
  }
);


// ─── Verificación de Pagos (Dueño / Admin) ─────────────────────

// GET — Dashboard de Verificaciones (Solo DUEÑO o ADMIN para visualización)
adminRoutes.get('/payments/verifications',
  requireRole('ADMIN', 'DUEÑO'), // Asumiendo que existe el rol DUEÑO
  async (req, res, next) => {
    try {
      ...
      const verifs = await prisma.verificacionPago.findMany({
        where,
        include: {
          pago: { include: { orden: true } },
          marcadoPor: { select: { nombre: true, apellido: true } },
          verificadoPor: { select: { nombre: true, apellido: true } }
        },
        orderBy: { createdAt: 'desc' }
      });
      res.json({ success: true, data: verifs });
    } catch (err) { next(err); }
  }
);

// POST — Admin marca pago como recibido
adminRoutes.post('/payments/:id/receive',
  requireRole('ADMIN'),
  async (req: AuthRequest, res, next) => {
    try {
      ...
      res.json({ success: true, data: result, message: 'Pago marcado como recibido. Notificación enviada al dueño.' });
    } catch (err) { next(err); }
  }
);

// POST — Dueño confirma verificación final
adminRoutes.post('/payments/verifications/:id/confirm',
  requireRole('DUEÑO', 'ADMIN'), // Permitimos ADMIN solo para propósitos de prueba o si el dueño delega
  async (req: AuthRequest, res, next) => {
    try {
      const verificacionId = parseInt(req.params.id);
      ...
      res.json({ success: true, data: result, message: 'Pago verificado y confirmado exitosamente.' });
    } catch (err) { next(err); }
  }
);

// POST — Dueño reversa verificación
adminRoutes.post('/payments/verifications/:id/reverse',
  requireRole('DUEÑO'),
  async (req: AuthRequest, res, next) => {
    try {
      const verificacionId = parseInt(req.params.id);
      ...
      res.json({ success: true, data: result, message: 'Verificación reversada correctamente.' });
    } catch (err) { next(err); }
  }
);

// GET — Auditoría Inmutable (Solo DUEÑO)
adminRoutes.get('/audit/immutable',
  requireRole('DUEÑO'),
  async (req, res, next) => {
    try {
...
// GET — ADMIN y GERENTE_VENTAS
adminRoutes.get('/cupones',
...
// POST — ADMIN y GERENTE_VENTAS
adminRoutes.post('/cupones',
  requireRole('ADMIN', 'GERENTE_VENTAS'),
  async (req, res, next) => {
  try {
    const datos = z.object({
      ...
      usosMaximos: z.number().int().positive().default(100),
    ...
    res.status(201).json({ success: true, data: cupon, message: 'Cupón creado' });
  } catch (err) { next(err); }
});

// PUT — ADMIN y GERENTE_VENTAS
adminRoutes.put('/cupones/:id',
  requireRole('ADMIN', 'GERENTE_VENTAS'),
  async (req, res, next) => {
  try {
    const id    = parseInt(req.params.id);
    ...
    // ✅ CORREGIDO: prisma.cupon (minúscula) — no prisma.Cupon
    ...
    res.json({ success: true, data: cupon, message: 'Cupón actualizado' });
  } catch (err) { next(err); }
});

// DELETE — solo ADMIN
adminRoutes.delete('/cupones/:id', requireRole('ADMIN'), async (req, res, next) => {
  try {
    const id = parseInt(req.params.id);
...
// POST /cupones/validar — cualquier rol autenticado
adminRoutes.post('/cupones/validar', async (req, res, next) => {
  try {
    const { codigo, subtotal } = z.object({
      codigo:   z.string(),
    ...
    if (!cupon)                         { res.status(404).json({ success: false, message: 'Cupón no encontrado' }); return; }
    if (!cupon.activo)                  { res.status(400).json({ success: false, message: 'Cupón inactivo' }); return; }
    if (new Date() < cupon.fechaInicio) { res.status(400).json({ success: false, message: 'Cupón aún no vigente' }); return; }
    if (new Date() > cupon.fechaFin)    { res.status(400).json({ success: false, message: 'Cupón expirado' }); return; }
    if (cupon.usos >= cupon.usosMaximos){ res.status(400).json({ success: false, message: 'Cupón agotado' }); return; }
    if (subtotal < Number(cupon.minimo)) {
      res.status(400).json({
        success: false,
        message: `Monto mínimo requerido: S/. ${Number(cupon.minimo).toFixed(2)}`,
      });
      return;
    }
...
// GET /inventory/movements — ADMIN, GERENTE_INVENTARIO
adminRoutes.get('/inventory/movements',
  requireRole('ADMIN', 'GERENTE_INVENTARIO'),
  async (req, res, next) => {
  try {
    const { pagina, porPagina, productoId } = z.object({
      pagina:     z.coerce.number().default(1),
...
// POST /inventory/movements — ADMIN, GERENTE_INVENTARIO
adminRoutes.post('/inventory/movements',
  requireRole('ADMIN', 'GERENTE_INVENTARIO'),
  async (req: AuthRequest, res, next) => {
  try {
    const datos = z.object({
    ...
    if      (datos.tipo === 'entrada') nuevoStock += datos.cantidad;
    else if (datos.tipo === 'salida')  nuevoStock -= datos.cantidad;
    else                               nuevoStock  = datos.cantidad;
    ...
    res.status(201).json({ success: true, data: movimiento, message: 'Movimiento registrado' });
  } catch (err) { next(err); }
});

// GET /inventory/stock — ADMIN, GERENTE_INVENTARIO, VENDEDOR (lectura)
adminRoutes.get('/inventory/stock',
  ...
  try {
    ...
    const resumen = {
      ...
      bajoStock:      productos.filter((p) => p.stock > 0 && p.stock <= p.stockMinimo).length,
      stockNormal:    productos.filter((p) => p.stock > p.stockMinimo).length,
    };

    res.json({ success: true, data: productos, resumen });
  } catch (err) { next(err); }
});
