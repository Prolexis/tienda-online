...
export default function CartPage() {
  const { carrito, obtenerCarrito, actualizarCantidad, eliminarItem, vaciarCarrito } = useCartStore()
  ...
  const handleCantidad = async (itemId: number, cantidad: number) => {
    ...
    try { await actualizarCantidad(itemId, cantidad) }
    catch { /* manejado */ }
  ...
  const checkStockAntesDeCheckout = async () => {
    ...
    try {
      // Re-obtener el carrito para asegurar stock fresco
      await obtenerCarrito()
      
      const hayProblemas = carrito.items.some(item => item.cantidad > item.producto.stock)
      if (hayProblemas) {
        toast.error('Algunos productos ya no tienen stock suficiente. Por favor revisa tu carrito.')
        return false
      }
      return true
    } catch {
  ...
  if (!usuario) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <h2 className="text-xl font-bold text-gray-800 mb-2">Necesitas iniciar sesión</h2>
        <p className="text-gray-500 mb-6">Para ver tu carrito debes estar autenticado</p>
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          {carrito.items.map((item) => (
            <div key={item.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex gap-4">
              ...
              <div className="w-20 h-20 flex-shrink-0 rounded-lg overflow-hidden bg-gray-100">
                <img
                  src={item.producto.imagen || 'https://via.placeholder.com/80x80/e5e7eb/9ca3af?text=?'}
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-primary-600 font-bold mt-1">S/. {item.precio.toFixed(2)}</p>
                ...
                <div className="flex items-center gap-3 mt-3">
                  ...
                  <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden">
                    <button
                      ...
                      className="px-3 py-1.5 bg-gray-50 hover:bg-gray-100 text-gray-700 font-bold transition-colors"
                    >-</button>
                ...
                <div className="mt-2">
                        {item.producto.stock === 0 ? (
                          <span className="text-xs font-medium text-red-600 bg-red-50 px-2 py-1 rounded-md">🚫 Sin stock disponible</span>
                        ) : item.producto.stock < item.cantidad ? (
                          <span className="text-xs font-medium text-orange-600 bg-orange-50 px-2 py-1 rounded-md">⚠️ Stock insuficiente (Quedan {item.producto.stock})</span>
                        ) : item.producto.stock <= 5 ? (
                          <span className="text-xs font-medium text-orange-500 bg-orange-50 px-2 py-1 rounded-md">⚡ ¡Últimas {item.producto.stock} unidades!</span>
                        ) : null}
                      </div>
                    </div>
                  </div>
          ...
          {/* Vaciar carrito */}
          <button onClick={vaciarCarrito} className="text-sm text-red-500 hover:text-red-700 transition-colors">
            🗑️ Vaciar carrito
          </button>
        </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 sticky top-24">
            <h2 className="font-bold text-lg text-gray-900 mb-5">Resumen del pedido</h2>
            ...
            <button
              ...
              disabled={validandoStock || carrito.items.some(item => item.cantidad > item.producto.stock)}
              className={`btn-primary w-full py-4 text-lg mt-6 shadow-lg shadow-primary-200 transition-all active:scale-[0.98] ${
                (validandoStock || carrito.items.some(item => item.cantidad > item.producto.stock)) ? 'opacity-50 cursor-not-allowed grayscale' : ''
              }`}
            >
              {validandoStock ? (
